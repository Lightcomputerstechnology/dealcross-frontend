...
import useTheme from '@/hooks/useTheme';           // ➜ realtime theme
...
export default function Navbar() {
  ...
  /* helpers for drawer styling */
  const drawerBg       = theme === 'dark' ? 'bg-gray-950 text-white'   : 'bg-white text-gray-900';
  const drawerBorder   = theme === 'dark' ? 'border-gray-700'          : 'border-gray-200';
  const overlayOpacity = theme === 'dark' ? 'bg-black/70'              : 'bg-black/40';
  ...
  return (
    <nav className={`bg-white dark:bg-gray-900 transition-colors duration-300 shadow-sm relative z-50 ${scrolled ? 'shadow-lg' : ''}`}>
        ...
        <div className="hidden md:flex space-x-6">
          {[
            ...
            <Link
              key={to}
              to={to}
              className={`hover:text-blue-600 dark:hover:text-blue-400 ${label === 'Upgrade' ? 'font-semibold' : ''}`}
            >
              {label}
            </Link>
          ))}
      ...
      <AnimatePresence>
        {open && (
          <>
            {/* overlay: opaque immediately, color based on theme */}
            <div
              className={`fixed inset-0 ${overlayOpacity} z-40`}
            ...
            <motion.div
              ...
              className={`fixed bottom-0 left-0 right-0 z-50 p-6 rounded-t-2xl shadow-lg space-y-6 text-lg font-medium ${drawerBg}`}
            >
              <div className="space-y-4">
                {[
                  ...
                  <Link key={to} to={to} onClick={() => setOpen(false)} className="block hover:text-blue-400">
                    {label}
                  </Link>
                ))}
              </div>

              <div className={`border-t pt-4 space-y-4 ${drawerBorder}`}>
