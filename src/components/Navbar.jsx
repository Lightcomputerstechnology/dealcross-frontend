...
export default function Navbar() {
  ...
  const theme = useTheme(); // detect live theme mode
    ...
    <nav className={`bg-white dark:bg-gray-900 relative z-50 transition-shadow duration-300 ${scrolled ? "shadow-lg" : ""}`}>
      ...
      <AnimatePresence mode="wait">
        {open && (
          <>
            <motion.div
              key={`overlay-${theme}`} // trigger full re-render on theme switch
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              key={`sidebar-${theme}`} // force re-render when theme changes
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
              transition={{ type: 'tween', duration: 0.25 }}
              className={`fixed bottom-0 right-0 left-0 rounded-t-xl shadow-lg z-50 p-6 space-y-6 text-lg font-medium
