...
const ChatWindow = ({ onClose, receiverId, dealId }) => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(
        `https://d-final.onrender.com/chat/messages/${receiverId}`,
        {
          headers: {
  ...
  return (
    <div className="fixed bottom-20 right-4 z-50 w-80 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl shadow-lg p-4">
      ...
      <div className="h-56 overflow-y-auto space-y-2 mb-3 px-1">
        {messages.length > 0 ? messages.map((msg) => (
          <div
            key={msg.id}
            className={`text-sm p-2 rounded-lg w-fit max-w-[70%] ${
              msg.sender_id === receiverId
                ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white ml-auto'
